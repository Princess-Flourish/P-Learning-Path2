...
def train_model(**context):
    ...
    try:
        ...
        try:
            ...
            tune_hyperparameters = False  # Default to True if variable doesn't exist
        ...
        training_results = {
            ...
            'feature_importance_top5': trainer.get_feature_importance(top_n=5).to_dict('records') if trainer.get_feature_importance() is not None else None
        }
        
        context['task_instance'].xcom_push(key='training_results', value=training_results)
        
        logger.info(f"Model training completed: {evaluation_metrics}")
